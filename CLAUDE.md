...
| 资源 | 路径 / 值 | 说明 |
|---|---|---|
| Qlib 数据（维护项目） | `/home/zxh/qlib_data` | 数据维护项目（含 .git/scripts/.ifind_token + 三核心子目录），用于数据下载/更新；不是 rdagent 直接读取的路径 |
...
# provider_uri 代码硬编码 ~/.qlib/qlib_data/cn_data，数据已复制到此路径（真实目录，见 §6.1）

# ==================== Factor CoSTEER 子进程 ====================
# 有意指向 0.8 共享 env，避免重复安装 torch/qlib（本条为设计意图标注）
...
9. 验收 → 墨菲定律全覆盖
10. 改后审查（强制）→ 见下方"改后审查清单"
11. 交付摘要 → 改了什么 / 验证证据 / 文件链接
```

### 改后审查清单（步骤 10，每次修改后强制执行）

完成任何代码/配置修改后，**必须**在交付前逐项确认：

| 审查项 | 必查内容 | 工具 |
|---|---|---|
| **A. 关联影响** | 本次改动是否波及其他模块？例如：改了某基类/接口/配置项，其调用方/子类/依赖方是否需要同步调整？改了数据 schema，读写双方是否都更新？ | CodeGraph `Show Callers` / `Show Impact`（§5.2 红线目录必用，其他目录推荐）+ grep 兜底 |
| **B. 文档同步** | 本次改动是否影响既有文档的准确性？例如：改了 CLI 命令 → 是否要同步 `CLAUDE.md` §12 自检命令？改了环境变量 → 是否要同步 §6.3 `.env` 模板？改了路径约定 → 是否要同步 §6.1 / §7？改了 API 签名 → 是否要同步 §3.1 核心组件表 / §7.2 适配矩阵？ | grep 文档中涉及的符号/路径/命令 |

**输出要求**：两项审查的结论必须显式写入交付摘要（§11），格式如 `关联影响：无 / 已处理 X 处` + `文档同步：无 / 已更新 §X.X`。**未声明这两项的交付视为流程不完整。**

> **第一性原理**：代码与文档是同一个系统的两个视图，任何让两者脱节的修改都是半成品。这条约束防止"改了代码忘改文档"和"修了 A 破坏 B"两类高频问题。
...
- **改后审查结论**（§9 步骤 10 强制）：交付摘要中必须显式声明两项——
...
# Qlib 数据可用性（qlib 在 Docker 内，主 env 不装；用 0.8 env 核对默认路径）
# 数据在 ~/.qlib/qlib_data/cn_data（真实目录，从 /home/zxh/qlib_data 复制的纯净数据）
/home/zxh/miniconda3/envs/rdagent/bin/python -c "import qlib; from qlib.constant import REG_CN; qlib.init(provider_uri='~/.qlib/qlib_data/cn_data', region=REG_CN); from qlib.data import D; cal=D.calendar(freq='day'); print(f'days={len(cal)} first={cal[0].date()} last={cal[-1].date()}')"
# 预期：days=6430 first=2000-01-04 last=2026-07-17
